...
# Connect constant position commands for iiwa arm
...
position_arm_source = builder.AddSystem(ConstantVectorSource(initial_positions))
builder.Connect(
    position_arm_source.get_output_port(),
    station.GetInputPort("iiwa_arm.position")
)
position_plate_source = builder.AddSystem(ConstantVectorSource(initial_positions))
builder.Connect(
    position_plate_source.get_output_port(),
    station.GetInputPort("iiwa_plate.position")
)

wsg_arm_source = builder.AddSystem(ConstantVectorSource([0.1]))  # 0.1m opening
builder.Connect(
    wsg_arm_source.get_output_port(),
    station.GetInputPort("wsg_arm.position")
)
wsg_plate_source = builder.AddSystem(ConstantVectorSource([0.1]))  # 0.1m opening
builder.Connect(
