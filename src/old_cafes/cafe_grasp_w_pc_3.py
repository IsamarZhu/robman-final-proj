...
def run_pick_place(meshcat, simulator, diagram, plant, plant_context,
    ...
    def ik_for_ee_position(p_W_target, use_orientation: bool,
        ...
        # --- lock the mobile base (robot_base_link) in place ---
        ...
        # Clamp z so we don't stab through the tray + mugs
        p = p_W_target.copy()
        p[2] = max(p[2], mug_bottom_z + 0.01)

        # Slightly looser position tolerance
        p_tol = 0.04   # was 0.02
        ik.AddPositionConstraint(
            ee_frame, [0, 0, 0],
            world_frame,
    ...
    # 2) Move straight down into the mug with position-only IK
    q_grasp = ik_for_ee_position(
        p_grasp,
        use_orientation=False,
    )

    # 3) Remaining waypoints as before
