...
def run_pick_place(meshcat, simulator, diagram, plant, plant_context,
    ...
    def ik_for_ee_position(p_W_target,
        ...
        # ⛔️ Orientation constraint removed for robustness
        # (we ignore use_orientation/theta_bound here on purpose)
        ...
        if not result.is_success():
            # Don't crash; just keep current arm configuration
            print(f"WARNING: IK failed for target {p_W_target}, "
                  f"using current arm configuration instead.")
    ...
    # Minimal: reuse over-mug posture for grasp
    q_grasp = q_over_m
