...
def remove_table_points(point_cloud: PointCloud,
    ...
    if xyz.shape[1] == 0:
        print("remove_table_points: input point cloud is empty.")
        return point_cloud
    ...
    if keep_idx.size == 0:
        print(f"remove_table_points: threshold {z_thresh} removed all points.")
        return PointCloud(0)  # explicitly empty
...
def keep_top_rim_band(pc: PointCloud,
    ...
    xyz = pc.xyzs()
    if xyz.shape[1] == 0:
        print("keep_top_rim_band: input point cloud is empty, returning unchanged.")
        return pc
    ...
    if idx.size == 0:
        print("keep_top_rim_band: no points in rim band, returning original pc.")
        return pc
...
def build_scene(meshcat):
    ...
    Also explicitly sets robot_base's free-body pose so the robot is upright,
    matching the 'base pose updated' script.
    """
    if running_as_notebook:
        mpld3.enable_notebook()

    # Load scenario.yaml (directives + cameras + model_drivers)
    ...
    # Add Rgbd cameras & point-cloud systems from scenario.cameras
    add_cameras(builder, plant, scene_graph, scenario)

    # Joint configs (match what you used before)
    ...
    # Keep grippers at zero torque so they don't fight anything
    ...
    # ---- HERE'S THE IMPORTANT PART: base pose ----
    robot_base_body = plant.GetBodyByName("robot_base_link", robot_base_instance)

    # Same pose you used in the working script ("base pose updated")
    initial_base_pose = np.array([
        -1.0, 0.0, 0.0, 0.0,   # quaternion (wxyz); -1 is same rotation as +1
        -3.5, 3.5, 0.8         # position
    ])
    ...
    # Now set joint positions to match
    ...
    # Force one render for cameras/visuals so point clouds are valid
...
def build_rim_pointcloud(meshcat, diagram, context) -> PointCloud:
    ...
    Be robust to over-aggressive z filtering.
    """
    ...
    print("Downsampled cloud: num points =", xyz_down.shape[1])
    if xyz_down.shape[1] == 0:
        raise RuntimeError("Downsampled point cloud is empty; check cameras / depth config.")

    print("Downsampled z min/max:", float(np.min(xyz_down[2, :])), float(np.max(xyz_down[2, :])))

    # 1) Try table removal with the nominal threshold
    obj_pc = remove_table_points(down_pc, z_thresh=TABLE_Z_THRESHOLD)

    if obj_pc.xyzs().shape[1] == 0:
        # If we removed everything, relax the threshold and/or skip this step
        print("Table removal nuked all points; relaxing threshold by 0.1.")
        obj_pc = remove_table_points(down_pc, z_thresh=TABLE_Z_THRESHOLD - 0.1)

        if obj_pc.xyzs().shape[1] == 0:
            print("Still empty after relaxed threshold; using full downsampled cloud.")
            obj_pc = down_pc

    xyz_obj = obj_pc.xyzs()
    print("After table removal: num points =", xyz_obj.shape[1])
    print("After table removal z min/max:", float(np.min(xyz_obj[2, :])), float(np.max(xyz_obj[2, :])))

    # 2) Keep only top band → mug rims, but don't allow it to go empty
    rim_pc = keep_top_rim_band(obj_pc, band_thickness=0.02)
    if rim_pc.xyzs().shape[1] == 0:
        print("Rim band cropping produced empty cloud; falling back to object_pc before rim crop.")
        rim_pc = obj_pc

    xyz_rim = rim_pc.xyzs()
    print("Rim cloud: num points =", xyz_rim.shape[1])
    print("Rim z min/max:", float(np.min(xyz_rim[2, :])), float(np.max(xyz_rim[2, :])))

    # Save and visualize
    np.save(SAVED_PC_PATH, xyz_rim)
...
def run_pick_place_with_ik(
    ...
    # 1) Position-only near the mug so it's easier to reach
    q_orient  = ik_for_ee_position(p_orient,   use_orientation=False)
    q_over_m  = ik_for_ee_position(p_over_mug, use_orientation=False)

    # 2) Only enforce top-down at the actual grasp, with a looser bound
    q_grasp   = ik_for_ee_position(
        p_grasp, use_orientation=True, theta_bound=0.8
    )

    # 3) After that, position-only for the rest
    q_lift    = ik_for_ee_position(p_lift,       use_orientation=False)
    q_over_t  = ik_for_ee_position(p_over_table, use_orientation=False)
    q_place   = ik_for_ee_position(p_place,      use_orientation=False)
    q_retreat = ik_for_ee_position(p_retreat,    use_orientation=False)
