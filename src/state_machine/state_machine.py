...
class CafeStateMachine:
    ...
    def navigate_move_to_waypoint_state(self):
        ...
        # Calculate current position with accel/decel
        # (current_x, current_y), (vx, vy), is_complete = self._update_movement_with_accel(dt)
        ...
        #         # More waypoints in this path
        #         self._start_rotation_to_waypoint()
        #     else:
        #         # Path complete, start table approach
    ...
    def _update_movement_with_accel(self, dt: float) -> Tuple[Tuple[float, float], Tuple[float, float], bool]:
        """
        Update position with acceleration/deceleration profile.
        Returns: ((x, y), (vx, vy), is_complete)
        """
        # Calculate distances for acceleration and deceleration phases
        accel_distance = (self.max_speed ** 2) / (2 * self.acceleration)
        decel_distance = (self.max_speed ** 2) / (2 * self.deceleration)
        
        # Adjust if total distance is too short
        if accel_distance + decel_distance > self.movement_distance:
            accel_distance = self.movement_distance / 2
            decel_distance = self.movement_distance / 2
            peak_speed = np.sqrt(self.acceleration * accel_distance)
        else:
            peak_speed = self.max_speed
        
        # Calculate timing
        accel_time = peak_speed / self.acceleration
        cruise_distance = self.movement_distance - accel_distance - decel_distance
        cruise_time = cruise_distance / peak_speed if peak_speed > 0 else 0
        decel_time = peak_speed / self.deceleration
        
        # Determine phase and calculate
        if dt < accel_time:
            # Acceleration
            self.current_speed = self.acceleration * dt
            distance_traveled = 0.5 * self.acceleration * dt**2
        elif dt < accel_time + cruise_time:
            # Cruise
            self.current_speed = peak_speed
            dt_cruise = dt - accel_time
            distance_traveled = accel_distance + peak_speed * dt_cruise
        elif dt < accel_time + cruise_time + decel_time:
            # Deceleration
            dt_decel = dt - accel_time - cruise_time
            self.current_speed = peak_speed - self.deceleration * dt_decel
            distance_traveled = accel_distance + cruise_distance + peak_speed * dt_decel - 0.5 * self.deceleration * dt_decel**2
        else:
            # Complete
            return self.target_position, (0.0, 0.0), True
        
        # Calculate current position
        progress = min(distance_traveled / self.movement_distance, 1.0)
        dx_total = self.target_position[0] - self.movement_start_pos[0]
        dy_total = self.target_position[1] - self.movement_start_pos[1]
        ...
        vx = direction_x * self.current_speed
        vy = direction_y * self.current_speed
        
        dist_to_target = np.sqrt((self.target_position[0] - current_x)**2 + 
                                 (self.target_position[1] - current_y)**2)
        
        if dist_to_target < self.position_tolerance:
            return self.target_position, (0.0, 0.0), True
        
        return (current_x, current_y), (vx, vy), False
